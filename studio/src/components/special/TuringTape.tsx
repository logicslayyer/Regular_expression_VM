...
export const TuringTape: React.FC = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '20px' }}>
      {/* Head indicator */}
      <div style={{ display: 'flex', marginBottom: '4px', paddingLeft: `${(tmHeadPosition - visibleStart) * 48}px`, transition: 'padding-left 0.3s ease' }}>
        <div style={{
      ...
      {/* Tape cells */}
      <div ref={containerRef} style={{ display: 'flex', overflowX: 'auto', border: '1px solid var(--border)', borderRadius: '6px', background: 'var(--bg-base)' }}>
        {/* Left ellipsis */}
        {visibleStart > 0 && (
